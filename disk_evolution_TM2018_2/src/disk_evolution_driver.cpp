...
void DiskEvolutionDriver::CalculateGasDustDragCoefficient()
{
    int is = pdata_->grid_.is_,
        ie = pdata_->grid_.ie_;
    double mfp, cSt, fmass, fsigmag, fsigmad, St_prime, A, Ainv, B;

    cSt = 0.5 * M_PI * pdata_->dust_.rho_di_ * pdata_->dust_.ad_;
    ...
    for (int i = is; i < ie; ++i) {
        ...
        pdata_->dust_.St_[i]     = (cSt / pdata_->gas_.Sigma_[i]) * std::max(1.0, 4.0*pdata_->dust_.ad_/(9.0*mfp));
