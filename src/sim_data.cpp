...
Dust::~Dust()
{
    if (is_allocate_arrays_) {
        array::Delete1dArray<double>(Sigma_);
        array::Delete1dArray<double>(Sigma_floor_);
        array::Delete1dArray<double>(md_);
        array::Delete1dArray<double>(ad_);
        array::Delete1dArray<double>(mSigma_);
        array::Delete1dArray<double>(St_);
        array::Delete1dArray<double>(Hd_);
        array::Delete1dArray<double>(vr_);
...
void Dust::SetDust(Utils::InputConfigure &input, Grid &grid)
{
    ...
    Sigma_       = array::Allocate1dArray<double>(nrtotc);
    Sigma_floor_ = array::Allocate1dArray<double>(nrtotc);
    md_          = array::Allocate1dArray<double>(nrtotc);
    ad_          = array::Allocate1dArray<double>(nrtotc);
    mSigma_      = array::Allocate1dArray<double>(nrtotc);
    St_          = array::Allocate1dArray<double>(nrtotc);
    Hd_          = array::Allocate1dArray<double>(nrtotc);
    vr_          = array::Allocate1dArray<double>(nrtotc);
...
void SimulationData::OutputData(std::string filename)
{
    ...
    for (int i = is; i < ie; ++i) {
        file.write((char*)&(dust_.md_[i]), sizeof(double));
    }

    for (int i = is; i < ie; ++i) {
        file.write((char*)&(dust_.ad_[i]), sizeof(double));
    }
